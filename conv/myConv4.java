...
public class myConv4
{
    public static void main(String[] args) {
        ...
        **/
        ...
        double[] output = conv(input, kernel, p, s, d, a);
        
        double[][] mat = convMat(input, kernel, p, s, d, a);
        double[][] mat_trans = convMat(output, rot(kernel), k-p-1, d, s, (i-k+2*p)%s);
    ...
    public static double[] conv(double[] input, double[] kernel, int p, int s, int d, int a) {
        ...
        for(int x=0; x<o; x++) {
            int v = x*s-p;
            for(int y = nm(max(v,0),d); y<min(k+v,i); y+=d) {
                output[x] += kernel[y-v]*input[y/d];
            }
        }
        return output;
    }
    
    public static int nm(int x, int d) {
        return (x%d==0) ? x : (x/d+1)*d;
    }
